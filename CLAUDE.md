...
| Health & Safety (public) | `healthSafety/` | page TBD (`HealthSafety.aspx`) | emergency steps + always-visible poster panels (PDFs pending upload to compass "Health and Safety" folder) + Tools (3 policies pending, JHSC Documents pending, 4 external Ontario/WSIB links) + JHSC roster from Employee Highlight `JHSCRole` rows (live) + guided incident report form (hero CTA + "Did an incident occur?" card; writes to the compass `Incident Reports` list (item-level lockdown); one tenant Power Automate flow emails HR on each new report, and there is NO Health & Safety mailbox (decided with HR); types include `Other`; harassment reports flagged Confidential, HR-only; submit failure falls back to contacting HR directly) |
| Incident Review (HR, unlisted) | `incidentReview/` | page TBD (`IncidentReview.aspx` on compass; deliberately NOT in the nav, reached from the HR notification email or a bookmark) | HR's queue over the `Incident Reports` list: a "How this works" 3-step strip, New/Filed/All filters, per report **Mark as filed / Reopen** (writes `HRReviewed` only, plus `ReleasedToHS=false` defensively) and **Export PDF** (`utils/exportReportPdf.ts`: opens a clean printable report in a new tab and the print dialog; values inserted via textContent; no PDF library); reviewer = Override List Behaviors on the list (PnP `PermissionKind.CancelCheckout`, Full Control/Design only, NOT Manage Lists) plus EditListItems, everyone else gets a read-only view of their own reports (query also scoped by `AuthorId`); harassment treated as confidential by TYPE as well as flag; shows the authenticated Author and last Editor; imports the compass SPFI from `healthSafety/services/spConfig` |
| HR Support (public) | `hrSupport/` | `/sites/compass/SitePages/HRSupport.aspx` (LIVE in nav) | itSupport structural clone, final shape: email-HR hero (no request form; `hrRequestService.ts` deleted), dark Tools toolbox (Employee Handbook + WorkSMART PDFs by UniqueId from the compass HR Support folder, ADP Internal Job Postings + Statutory Holidays external links), Other HR contacts list (no phone numbers by HR request) |
...
| All About the Company (public) | `aboutCompany/` | pages TBD, one per section via the `section` web part property (`MissionVisionValues.aspx`, `CompanyHistory.aspx`, `SeniorLeadership.aspx`, `QualityManagementSystem.aspx`; 'all' renders the original single page) | MVV as the branding team's own 1-pager images (bundled assets from the Management toolkit; full text kept sr-only for AODA) + video slot (`videoUrl` web part property; launches 2026-09-14) + History placeholder + senior leadership derived from Employee Highlight `Management` department tags (Management-only rows sort first as executives) with short bios via optional `LeadershipBio` column (pending creation; defensive separate read) + Leadership Updates via the Announcements list (`Page=Leadership` choice value pending) + QMS section (ISO 9001:2015 statement, Quality Policy, objectives, why-unique + 3 docs from the compass 'Quality Management System' folder by UniqueId, H&S-style Tools viewer); reads via `employeeDirectory` spConfig + `rapidCityHomepage` announcements service |
...
- **`Navigation` (in rapidCityHomepage) imports from `customerContactCards`** (`NotificationBell`, `useSearchCustomers`) **and `outsourceContactCards`** (`useSearchVendors`) plus `WeatherWidget`. Every page that renders Navigation transitively depends on both Contact Cards web parts. The nav search bar renders ONLY on the two card pages (`activePage` contactCards/outsourceCards) with a typeahead over that page's own data (customers vs vendors, via `onCustomerSelect`/`onVendorSelect`); all other pages have no nav search. "Contact Cards" in the nav is a dropdown (Customer / Outsource); "All About the Company" is a section dropdown linking one PAGE per section (CompanyHistory / SeniorLeadership / MissionVisionValues / QualityManagementSystem .aspx on compass, each hosting aboutCompany with its `section` property; disabled until launch); Health & Safety exists but is greyed until launch; HR Support is LIVE. Employee Support gains a **Management Site** link only for viewers who can open `/sites/Management` (probed with `currentUserHasPermissions(ViewPages)` via `Web([sp.web, url])`, sessionStorage caches only a real answer or a 401/403, fails hidden). `employeeDirectory` now initializes the Contact Cards SPFI like every other Navigation host. Vivetta (Palantir Foundry, external) is LIVE: gradient-mark + label item beside Home (plus a mobile drawer entry), opens in a new tab - NOT a dropdown item, per IT.
...
One flow is live in the tenant for the compass `Incident Reports` list (built by the user in the portal, not in this repo): **Incident Reports - Notify HR on new report** (trigger: item created -> email the HR mailbox; its "Review it" link should point at `IncidentReview.aspx`). The former **Incident Reports - Release to Health & Safety** flow is RETIRED along with the H&S mailbox (turn it off in the portal); `ReleasedToHS`/`HSNotified` columns are unused and may be deleted, and the code tolerates either.
