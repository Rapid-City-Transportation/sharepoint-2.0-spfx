...
| Health & Safety (public) | `healthSafety/` | page TBD (`HealthSafety.aspx`) | emergency steps + always-visible poster panels (PDFs pending upload to compass "Health and Safety" folder) + Tools (3 policies pending, JHSC Documents pending, 4 external Ontario/WSIB links) + JHSC roster from Employee Highlight `JHSCRole` rows (live) + guided incident report form (hero CTA + "Did an incident occur?" card; writes to the compass `Incident Reports` list (item-level lockdown); HR-first routing + H&S-mailbox release via two tenant Power Automate flows; harassment reports flagged Confidential, HR-only; submit failure falls back to contacting HR directly) |
| Incident Review (HR, unlisted) | `incidentReview/` | page TBD (`IncidentReview.aspx` on compass; deliberately NOT in the nav, reached from the HR notification email or a bookmark) | HR's review queue over the `Incident Reports` list: release to Health & Safety (confirmed first) or keep with HR, instead of ticking raw checkboxes; confidential reports never offer release; reviewer = Override List Behaviors on the list (PnP `PermissionKind.CancelCheckout`; the permission that lifts item-level restrictions, shipped in Full Control/Design only, NOT Manage Lists) plus EditListItems, everyone else gets a read-only view of their own reports (query also scoped by `AuthorId`); harassment is treated as confidential by TYPE as well as by flag; shows the authenticated Author and last Editor because submitters can edit their own rows (operational gap: a submitter could tick the review columns on their own item, so Flow 2 should also check who modified); only ever sets `HRReviewed`/`ReleasedToHS` (never `Confidential` or the flow-owned `HSNotified`); imports the compass SPFI from `healthSafety/services/spConfig` |
...
Two flows DO exist in the tenant for the compass `Incident Reports` list (built by the user in the portal, not in this repo): **Incident Reports - Notify HR on new report** (trigger: item created -> email the HR mailbox) and **Incident Reports - Release to Health & Safety** (trigger: item created or modified; condition `HRReviewed = true AND ReleasedToHS = true AND Confidential = false AND HSNotified = false` -> email the H&S mailbox, then set `HSNotified = true`). The `incidentReview` page drives the second flow by setting the two review columns; code must never write `Confidential` (after submission) or `HSNotified`.
