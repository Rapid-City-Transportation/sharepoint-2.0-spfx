...
import styles from './FleetHub.module.scss';
import { IFleetHubProps } from './IFleetHubProps';
import {
  defaultTheme,
...
const DEPARTMENT_NAME = 'Fleet';

// Public Fleet page on compass; must be created with exactly this name.
const PUBLIC_PAGE_URL = 'https://rapidcitytransport.sharepoint.com/sites/compass/SitePages/Fleet.aspx';
...
/** Private Fleet hub page: updates from the 'Fleet Private' announcements
...
const FleetHub: React.FC<IFleetHubProps> = ({ bannerEyebrow, bannerTitle }) => {
  const themeVars = React.useMemo(
    () => getThemeCssVariables(defaultTheme) as React.CSSProperties,
    []
  );

  const { announcements: updates } = useAnnouncements('Fleet Private');
  ...
  const renderToolsPanel = (suffix: string, panelClass: string): React.ReactNode => (
    <section
      ...
      aria-labelledby={`fleet-tools-title${suffix}`}
    >
      <div className={styles.panelHeader}>
        <h3 id={`fleet-tools-title${suffix}`} className={styles.panelTitle}>
    ...
    <div className={styles.hub} style={themeVars}>
      ...
      <div className={styles.layout}>
        <a href={PUBLIC_PAGE_URL} className={styles.backLink}>
          ← Back to the public Fleet page
        </a>

        <main className={styles.mainColumn}>

          <section className={styles.updatesCard} aria-labelledby="fleet-updates-title">
            <div className={styles.updatesHeader}>
              <div className={styles.updatesHeaderText}>
                ...
                <h2 id="fleet-updates-title" className={styles.updatesTitle}>
                  {bannerTitle}
                </h2>
              </div>
            </div>
            ...
            {updates.length === 0 ? (
              <p className={styles.updatesEmpty}>
                No updates yet. Announcements for Fleet will appear here.
              </p>
            ) : (
          ...
          <section className={styles.toolViewerCard} aria-labelledby="fleet-viewer-title">
            <header className={styles.toolViewerHeader}>
              <div className={styles.toolViewerHeaderLeft}>
                ...
                <div>
                  ...
                  <h3 id="fleet-viewer-title" className={styles.toolViewerTitle}>
                    Select a tool
                  </h3>
                </div>
              </div>
          ...
          <section className={styles.teamCard} aria-labelledby="fleet-team">
            <div className={styles.teamHeader}>
              <div>
                <h2 id="fleet-team" className={styles.teamTitle}>Senior &amp; Support Team</h2>
              </div>
            </div>
            ...
            {!teamLoading && members.length === 0 && (
              <p className={styles.teamMemberRole}>
                ...
                Show In Dept Team for the Fleet people you want here.
              </p>
            )}
            {members.length > 0 && (
