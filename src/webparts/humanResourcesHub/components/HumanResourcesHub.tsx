...
import styles from './HumanResourcesHub.module.scss';
import { IHumanResourcesHubProps } from './IHumanResourcesHubProps';
import {
  defaultTheme,
...
// Public Human Resources page on compass; must be created with exactly this name.
const PUBLIC_PAGE_URL = 'https://rapidcitytransport.sharepoint.com/sites/compass/SitePages/HumanResources.aspx';
...
/** Private Human Resources hub page: updates from the 'Human Resources Private' announcements
...
const HumanResourcesHub: React.FC<IHumanResourcesHubProps> = ({ bannerEyebrow, bannerTitle }) => {
  const themeVars = React.useMemo(
    () => getThemeCssVariables(defaultTheme) as React.CSSProperties,
    []
  );

  const { announcements: updates } = useAnnouncements('Human Resources Private');
  ...
  const renderToolsPanel = (suffix: string, panelClass: string): React.ReactNode => (
    <section
      ...
      aria-labelledby={`hr-tools-title${suffix}`}
    >
      <div className={styles.panelHeader}>
        <h3 id={`hr-tools-title${suffix}`} className={styles.panelTitle}>
    ...
    <div className={styles.hub} style={themeVars}>
      ...
      <div className={styles.layout}>
        <a href={PUBLIC_PAGE_URL} className={styles.backLink}>
          ← Back to the public Human Resources page
        </a>

        <main className={styles.mainColumn}>

          <section className={styles.updatesCard} aria-labelledby="hr-updates-title">
            <div className={styles.updatesHeader}>
              <div className={styles.updatesHeaderText}>
                ...
                <h2 id="hr-updates-title" className={styles.updatesTitle}>
                  {bannerTitle}
                </h2>
              </div>
            </div>
            ...
            {updates.length === 0 ? (
              <p className={styles.updatesEmpty}>
                No updates yet. Announcements for Human Resources will appear here.
              </p>
            ) : (
          ...
          <section className={styles.toolViewerCard} aria-labelledby="hr-viewer-title">
            <header className={styles.toolViewerHeader}>
              <div className={styles.toolViewerHeaderLeft}>
                ...
                <div>
                  ...
                  <h3 id="hr-viewer-title" className={styles.toolViewerTitle}>
                    Select a tool
                  </h3>
                </div>
              </div>
          ...
          <section className={styles.teamCard} aria-labelledby="hr-team">
            <div className={styles.teamHeader}>
              <div>
                <h2 id="hr-team" className={styles.teamTitle}>Senior &amp; Support Team</h2>
              </div>
            </div>
            ...
            {!teamLoading && members.length === 0 && (
              <p className={styles.teamMemberRole}>
                ...
                Show In Dept Team for the Human Resources people you want here.
              </p>
            )}
            {members.length > 0 && (
...
export default HumanResourcesHub;
