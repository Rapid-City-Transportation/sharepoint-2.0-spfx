...
import styles from './AccountingHub.module.scss';
import { IAccountingHubProps } from './IAccountingHubProps';
import {
  defaultTheme,
...
// Public Accounting page on compass; must be created with exactly this name.
const PUBLIC_PAGE_URL = 'https://rapidcitytransport.sharepoint.com/sites/compass/SitePages/Accounting.aspx';
...
/** Private Accounting hub page: updates from the 'Accounting Private' announcements
...
const AccountingHub: React.FC<IAccountingHubProps> = ({ bannerEyebrow, bannerTitle }) => {
  const themeVars = React.useMemo(
    () => getThemeCssVariables(defaultTheme) as React.CSSProperties,
    []
  );

  const { announcements: updates } = useAnnouncements('Accounting Private');
  ...
  const renderToolsPanel = (suffix: string, panelClass: string): React.ReactNode => (
    <section
      ...
      aria-labelledby={`accounting-tools-title${suffix}`}
    >
      <div className={styles.panelHeader}>
        <h3 id={`accounting-tools-title${suffix}`} className={styles.panelTitle}>
    ...
    <div className={styles.hub} style={themeVars}>
      ...
      <div className={styles.layout}>
        <a href={PUBLIC_PAGE_URL} className={styles.backLink}>
          ← Back to the public Accounting page
        </a>

        <main className={styles.mainColumn}>

          <section className={styles.updatesCard} aria-labelledby="accounting-updates-title">
            <div className={styles.updatesHeader}>
              <div className={styles.updatesHeaderText}>
                ...
                <h2 id="accounting-updates-title" className={styles.updatesTitle}>
                  {bannerTitle}
                </h2>
              </div>
            </div>
            ...
            {updates.length === 0 ? (
              <p className={styles.updatesEmpty}>
                No updates yet. Announcements for Accounting will appear here.
              </p>
            ) : (
          ...
          <section className={styles.toolViewerCard} aria-labelledby="accounting-viewer-title">
            <header className={styles.toolViewerHeader}>
              <div className={styles.toolViewerHeaderLeft}>
                ...
                <div>
                  ...
                  <h3 id="accounting-viewer-title" className={styles.toolViewerTitle}>
                    Select a tool
                  </h3>
                </div>
              </div>
          ...
          <section className={styles.teamCard} aria-labelledby="accounting-team">
            <div className={styles.teamHeader}>
              <div>
                <h2 id="accounting-team" className={styles.teamTitle}>Senior &amp; Support Team</h2>
              </div>
            </div>
            ...
            {!teamLoading && members.length === 0 && (
              <p className={styles.teamMemberRole}>
                ...
                Show In Dept Team for the Accounting people you want here.
              </p>
            )}
            {members.length > 0 && (
